...
export default function BookmarkManager({ user }: { user: any }) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-8">
      {/* Input Form */}
      <form onSubmit={addBookmark} className="mb-8 p-6 bg-white border border-gray-200 rounded-lg shadow-sm flex gap-3">
        <input 
          type="text" 
          placeholder="Title" 
          value={title} 
        ...
        <input 
          ...
          className="border border-gray-300 p-3 rounded-md flex-1"
        />
        <button type="submit" className="bg-blue-600 text-white px-6 rounded-md">Add</button>
      </form>

      {/* List */}
      <ul className="space-y-3">
        {bookmarks.map((bm) => (
          <li key={bm.id} className="p-4 bg-white border rounded shadow flex justify-between">
            <a href={bm.url} target="_blank" className="text-blue-600 font-bold">{bm.title}</a>
            <button onClick={() => deleteBookmark(bm.id)} className="text-red-500">Delete</button>
          </li>
        ))}
      </ul>
    </div>
